...
```{r}
rMano <- function(k = 5){
  Ran <- rep(c("A", 2: 10, "J", "Q", "K"),4) #repite 4 veces la secuencia de 13 cartas
...
```{r}
plot(X1a, X1b) 
#X1a puede ser TRUE o FALSE
#X1b puede ser TRUE o FALSE
# combinación: TT, TF, FT, FF
```
...
```{r}
X4a <- replicate(1000, niReyniReina(Rango(rMano(2))))
```

```{r}
table(X4a)/length(X4a)
#FALSE: salio un rey o una reina, al menos
#TRUE: no salio ni un rey y tampoco una reina
```
...
Ejercicio 1
```{r}
dosdados6 <- replicate(500000, sum(replicate(2, rDado()) == 6)==2)
table(dosdados6)/length(dosdados6)
```
...
victoria<-function()
{
tresdados <- replicate(3, rDado())
#print(tresdados)
x<-"FALSE"
if (sum(tresdados>=3)>=2)
   {if (sum(tresdados>=4)>=1)
     x<-"TRUE"} 
return(x)
}


juego<-replicate(10000,victoria())

table(juego)/length(juego)
```
