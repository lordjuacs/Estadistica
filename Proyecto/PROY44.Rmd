...
Según la tabla podemos observar una aglomeración por parte de los estudiantes que tienen 18 años de entre 50 y 100 minuutos diarios. Mientras que para los estudiantes de 22 años se puede observar una aglomeración de entre 0 y 50 minutos.
...
chisq.test(t_e)
```
No se puede rechazar la hipótesis nula ya que nuesto nuesto alpha es mayor a nuestro p-value, **0.2262>0.05**.
...
Utilizaremos el valor de estres medio, **48**, para usarlo como punto medio para los estudiantes que tienen un estrés mayo o menor que la mayoría.
...
En la gráfica se puede notar una cierta tendencia negativa del promedio con respecto al aumento en el tiempo de transporte diario para los estudiantes. Esta hipótesis se apoya en que para un timepo de transporte de 50 minutos diarios se observa una aglomeración en un promedio de 16, mientras que para un tiempo de 200 minutos diarios se observa una aglomeración en 12 de promedio. 
...
**ANÁLISIS EXPLORATORIO:** Dado los datos obtenidos de la gráfica podemos notar una tendencia de un mayor tiempo de transporte semanal con la probabilidad de reprobar uno o mas cursos ya que los estudiantes que han reprobado al menos un curos tienen una media de tiempo de transporte semanal significativamente mas alta.
...
**Comparando el tiemo de transporte semanal de alumnos que han reprobado uno o mas cursos y los alumnos qu no cuentan con cursos reprobados**
```{r}
...
Finalmente observando el r cuadrado se tiene que el nivel de estrés explica el 7.6% de la variación en las horas de sueño diarias. Además con el análisis de T-Student se confirma que la regresión y la relación entre las variables no es significativa por lo que se concluye que el modelo no es adecuado para realizar predicción sobre la variable.
...
Al analizar el modelo obtenido se observa un p.value igual a 0.2591, por lo que al ser mayor a la significancia no se rechaza la hipótesis nula. Por lo que se tiene evidencia estadística para establecer la no dependencia del número de reprobados con respecto al tiempo de transporte diario.
Asimismo, al observar la tendencia (resultado de la regresión) en el gráfico se puede concluir que el modelo y la tendencia no son lo suficiente buenos para establecer una relación entre las variables.
Finalmente con el análisis de T-Student se confirma que la regresión y la relación entre las variables no es significativa por lo que se concluye que el modelo no es adecuado para realizar predicción sobre la variable.
...
Asimismo, en vista del valor del r cuadrado (0.04969) y la heterocedasticidad de los errores se concluye que el modelo no permite explicar la variación de la variable de manera adecuada, por lo que es insuficiente para realizar predicción sobre esta variable.
Finalmente con el análisis de T-Student se confirma que la regresión y la relación entre las variables no es significativa por lo que se concluye que el modelo no es adecuado para realizar predicción sobre la variable.
...
A pesar de todo esto, al observar el gráfico de los residuos del modelo se observa heterocedasticidad y en el gráfico de la normal se observan los valores muy dispersos de la tendencia que deberían seguir. Además con  el análisis de T-Student se observa que la regresión y la relación entre las variables no es significativa por lo que se concluye que el modelo no es adecuado para realizar predicción sobre la variable.
...
Finalmente el análisis de T-Student confirma que la regresión y la relación entre las variables no es significativa; por lo que no nos puede decir nada sobre una predicción de las mismas.
...
Asimismo, al obtenerse todos los p.values de la variable medio de transporte mayores a la significancia. No se puede rechazar la hipótesis nula, la cual establece la no dependencia de la variable dependiente con respecto a esta.
Además, dado el valor del r cuadrado se puede decir que el modelo explica el 38.84% de la variación de la variable dependiente. 
Finalmente, en el gráfico de residuos del modelo se puede observa heterocedasticidad. Además de que en el análisis de T-Student se confirma que la regresión y la relación entre las variables no es significativa por lo que se concluye que el modelo no es adecuado para realizar predicción sobre la variable.
...
**#** Respecto a los modelos obtenidos por regresión, no se encontró ninguno que sea lo suficientemente robusto como para realizar predicción sobre alguna variable.
